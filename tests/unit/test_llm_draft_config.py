...
# Should inherit draft_editor from [llm]
...
def test_draft_editor_inheritance(draft_llm_toml):
    """Test that draft_editor is correctly handled in different scenarios:
    - Inherited from generic [llm] section
    - Overridden in custom section
    ...
    # Verify custom1 inherits draft_editor from generic
...
def test_draft_editor_defaults(draft_llm_toml):
    """Test that draft_editor inherits default values from LLMConfig when not specified"""
    config = AppConfig()
    load_from_toml(config, draft_llm_toml)
