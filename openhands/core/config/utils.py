                ...
                if key is not None and key.lower() == 'agent':
                    ...
                    for nested_key, nested_value in value.items():
                        if isinstance(nested_value, dict):
                            ...
                            cfg.set_llm_config(custom_llm_config, nested_key)                    # Extract generic LLM fields, keeping draft_editor
                    generic_llm_fields = {}
                    for k, v in value.items():
                        if not isinstance(v, dict) or k == 'draft_editor':
                            generic_llm_fields[k] = v
                    ...
                    for nested_key, nested_value in value.items():
                        if isinstance(nested_value, dict):
                            ...
                            custom_llm_config = LLMConfig.from_dict(merged_llm_dict)
                            cfg.set_llm_config(custom_llm_config, nested_key)                elif key is not None and key.lower() == 'security':
                    logger.openhands_logger.debug(
